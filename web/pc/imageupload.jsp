  ...
  */
				...
				fos.close();
                      ...
                      //File file = new File("myPhoto.png");
                         File in=new File("C:\\Users\\Prakash\\Desktop\\Out" + filename);
                      FileInputStream fis = new FileInputStream(in);
                      
                       ps = fileupload.con.prepareStatement(INSERT_PICTURE);
                      ps.setString(1, responseJson.getString("username"));
                      ps.setInt(2, Integer.parseInt(responseJson.getString("cmd_id")));
                      ps.setString(3, "Screenshot.png");
                      ps.setString(4, "No_file_path");
                      ps.setString(5, "screenshot");
                      ps.setBinaryStream(6, fis, (int) in.length());
                      int check=  ps.executeUpdate();
                      DatabaseCon db = new DatabaseCon();
                      ResultSet rs = db.getResult("SELECT * FROM `filestore` WHERE file_cmd_id = '"+responseJson.getString("cmd_id")+"'");
                      if(rs.next())
                       {        
                        String imgCmdId = rs.getString("file_id");
                      String queryString = "UPDATE `commands` SET cmd_send_state = '1', cmd_ack = '1', cmd_exec = '1', cmd_extra_id = '"+imgCmdId+"' WHERE cmd_id = '"+responseJson.getString("cmd_id")+"'";
